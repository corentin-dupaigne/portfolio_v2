...
More information: [Netfilter hooks and packet flow in detail (thermalcircle.de)](https://thermalcircle.de/doku.php?id=blog:linux:nftables_packet_flow_netfilter_hooks_detail)
...
![The five netfilter hooks along the packet path](20260922-174238.png)
...
For a ClusterIP, the rewrite that matters is DNAT at the `PREROUTING` hook. The client connects to the virtual IP, and DNAT swaps the destination to a chosen pod's real IP *before routing* (it must be pre-routing, since the new destination decides where the packet is routed).
...
```bash
sudo iptables -t nat -A PREROUTING -d 10.96.72.11 -p tcp --dport 80 \
  -j DNAT --to-destination 10.244.0.9:8080
```
...
1. We created a simple deployment app composed of 3 replicas.
   - Pods IPs: `10.244.0.3`, `10.244.0.4`, `10.244.0.5`

   ![The three pod replicas and their IPs](20260922-174251.png)

2. Then we exposed a ClusterIP for the deployment.
   - Virtual ClusterIP IP: `10.103.181.160`

   ![The ClusterIP Service and its virtual IP](20260922-174300.png)
...
![Rules in the KUBE-SERVICES chain](20260922-174309.png)
...
![Rules in the service's KUBE-SVC chain](20260922-174358.png)

Remember, rules are evaluated in order (top to bottom).

The first rule says that there is a probability of 1/3 that the first replica of our app deployment (`10.244.0.3`) will be the one receiving the packet.

- Pod 1: 1/3

Then if we go to the second rule it means the first pod has not received the packet, therefore there are 2 pods left. So we eliminate the first one for the draw and there is now a probability of 1/2 that the second replica (`10.244.0.4`) will be chosen.
...
![DNAT rule in the first replica's KUBE-SEP chain](20260922-174406.png)
